...
class CropPredictionApp:
    def __init__(self, root):
        ...
        self.preview_size = self._calculate_preview_size()
    ...
    def setup_main_area(self):
        ...
        self.img_container.grid_columnconfigure(1, weight=1)
        self.img_container.grid_columnconfigure(2, weight=1)
    ...
    def create_img_slot(self, parent, text, col):
        ...
        lbl = tk.Label(frame, bg="#ecf0f1", width=w, height=h, relief="ridge", bd=1)
        lbl.pack()
        return lbl

    def _calculate_preview_size(self):
        # tkinter Label width/height are in text units, not pixels.
        # Increased preview window to show larger original and preprocess outputs.
        if self.w >= 1900:
            return (58, 20)
        if self.w >= 1500:
            return (50, 18)
    ...
    def _display_size_px(self):
        # Actual image render size used by PIL resize before displaying.
        if self.w >= 1900:
            return (500, 360)
        if self.w >= 1500:
            return (440, 320)
    ...
    def openimage(self):
        ...
        if fileName:
            self.fn = fileName
            img = Image.open(self.fn).resize(self._display_size_px())
    ...
    def convert_grey(self):
        ...
        gray_disp = cv2.resize(gray, self._display_size_px())
